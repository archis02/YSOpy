...
def func(dict_config):
    ...
    print("phot done")
    total_flux = bf.dust_extinction_flux(dict_config, wavelength, obs_viscous_disk_flux,
...
def calc_continuum_arr(theta: np.ndarray, spec_wavelength: np.ndarray, flux: np.ndarray, threshold_l = 0.9, plot=False):
    """
...
if __name__ == '__main__':
    ...
    wv_right_line, hslab_flux_right_line = trim_in_window(wavelength=wave, total_flux=hslab_flux, window=window)
    wv_right_line, bbody_flux_right_line = trim_in_window(wavelength=wave, total_flux=bbody_flux, window=window)
    ...
    if plot_bool:
        ...
        plt.plot(wave_left, conti_l_hs, label="Fitted-Continuum for H-Slab")
        plt.plot(wave_left, conti_l_bb, label="Fitted-Continuum for Blackbody")
        plt.xlabel("Wavelength in Angstrom")
        plt.ylabel("Normalised Flux")
        plt.ylim((0.8, 1.1))
        plt.text(8184, 0.845, f"Eq width (Hslab): {(eq_width_l_hs.value):.3f} Ang")
        plt.text(8184, 0.83, f"Eq width (Blackbody): {(eq_width_l_bb.value):.3f} Ang")
        plt.grid()
    ...
    if plot_bool:
        ...
        plt.ylabel("Normalised Flux")
        plt.ylim((0.8, 1.1))
        plt.text(8328, 0.845, f"Eq width (Hslab): {(eq_width_r_hs.value):.3f} Ang")
        plt.text(8328, 0.83, f"Eq width (Blackbody): {(eq_width_r_bb.value):.3f} Ang")
        plt.legend(loc="upper left")
        plt.grid()
